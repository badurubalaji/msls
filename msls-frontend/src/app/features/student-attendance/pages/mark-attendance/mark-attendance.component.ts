...
@Component({
  ...
  template: `
    <div class="container mx-auto px-4 py-6">
      <!-- Header -->
      <div class="mb-6">
        <h1 class="text-2xl font-semibold text-gray-900">Mark Student Attendance</h1>
        <p class="text-sm text-gray-600 mt-1">Mark daily attendance for your classes</p>
      </div>
      ...
      @if (!loadingClasses() && !error()) {
        <div class="bg-white rounded-lg shadow-sm border p-4 mb-6">
          <div class="flex flex-wrap gap-4 items-end">
            ...
            <!-- Submit Button -->
            @if (classAttendance() && classAttendance()!.students.length > 0) {
              <button
              ...
              >
                ...
                {{ classAttendance()!.isMarked ? 'Update' : 'Submit' }} Attendance
              </button>
            }
          </div>
        </div>
      ...
      @if (classAttendance() && !loadingAttendance()) {
        <div class="bg-white rounded-lg shadow-sm border">
          ...
          <div class="p-4 border-b flex flex-wrap gap-4 items-center justify-between">
            ...
            <div class="flex gap-4 text-sm">
              <span class="text-gray-600">Total: {{ classAttendance()!.summary.total }}</span>
          ...
          <!-- Status Info -->
          @if (classAttendance()!.isMarked && !classAttendance()!.canEdit) {
            <div class="px-4 py-2 bg-yellow-50 border-b text-yellow-800 text-sm">
              Attendance was marked at {{ formatDateTime(classAttendance()!.markedAt) }} by {{ classAttendance()!.markedByName || 'Unknown' }}.
            ...
            <table class="w-full">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider w-28">Last 5 Days</th>
              ...
              <tbody class="divide-y divide-gray-200">
                @for (student of filteredStudents(); track student.studentId; let i = $index) {
                  ...
                  >
                    ...
                    <td class="px-4 py-3">
                      <div class="flex justify-center gap-1">
                        @for (day of student.last5Days; track $index) {
                          <span
                            [class]="getDotColor(day)"
                            class="w-3 h-3 rounded-full"
                            [title]="getStatusLabel(day)"
                          ></span>
                        }
                        @for (i of getEmptyDots(student); track i) {
...
export class MarkAttendanceComponent implements OnInit {
  ...
  classAttendance = signal<ClassAttendance | null>(null);
  ...
  presentCount = computed(() => {
    const attendance = this.classAttendance();
  ...
  loadClassAttendance(): void {
    ...
    this.attendanceService.getClassAttendance(this.selectedSectionId(), this.selectedDate()).subscribe({
      next: (attendance) => {
        ...
        this.classAttendance.set(attendance);
        this.loadingAttendance.set(false);
      },
      error: (err) => {
        this.loadingAttendance.set(false);
        this.error.set(err.error?.message || 'Failed to load class attendance');
      }
    });
  }
  ...
  setStudentStatus(student: StudentForAttendance, status: StudentAttendanceStatus): void {
    if (!this.canEdit()) return;
    student.status = status;
    // Trigger change detection
    this.classAttendance.update(a => a ? { ...a } : null);
  }
  ...
  submitAttendance(): void {
    ...
    const records: AttendanceRecordRequest[] = attendance.students.map(s => ({
      studentId: s.studentId,
      status: (s.status || 'present') as StudentAttendanceStatus,
      remarks: s.remarks || undefined,
    }));

    const request: MarkClassAttendanceRequest = {
      date: this.selectedDate(),
      records,
    };

    this.attendanceService.markClassAttendance(this.selectedSectionId(), request).subscribe({
      next: (result) => {
  ...
  getStatusButtonClass(student: StudentForAttendance, status: StudentAttendanceStatus): string {
  ...
  getDotColor(status: string): string {
    return STUDENT_ATTENDANCE_DOT_COLORS[status] || 'bg-gray-300';
  }

  getStatusLabel(shortStatus: string): string {
    ...
    return mapping[shortStatus] || shortStatus;
  }

  getInitials(student: StudentForAttendance): string {
    return (student.firstName[0] || '') + (student.lastName[0] || '');
  }

  getEmptyDots(student: StudentForAttendance): number[] {
    const count = 5 - (student.last5Days?.length || 0);
